/**
 * 游戏状态管理 v0.8
 * P4强化：calculateParallelHooks增加7个新状态检查（noHouse/noChildren/noFriends/highSavingsLowInvest/noLearning等）
 */
...
class GameState {
  ...
  advanceMonth() {
    ...
    // 4.5 处理职业延迟（考研/Gap期间无工资，结束后应用工资起点倍率）
    ...
    // 5.6 房产更新（还贷+房价波动）
    if (this.propertyManager && this.player) {
      this.propertyManager.updateMortgages(this.player);
      this.propertyManager.updatePropertyPrices(this.player);
    }
    // 6. 记录月度数据
  ...
  triggerRandomRelationshipEvent() {
    ...
    const eventTypes = [];
    // 朋友事件
    if (rm.friends.length > 0) {
      eventTypes.push('friend_help', 'friend_opportunity');
    }
    // 认识新朋友
    if (rm.friends.length < 10 && this.player.network > 20) {
      eventTypes.push('make_friend');
    }
    // 伴侣事件
    if (rm.partner && rm.partner.status === 'dating') {
      eventTypes.push('date_reminder');
    }
    // 生育机会
    ...
    if (eventType === 'make_friend') {
      ...
      if (helpEvent) {
        this.pendingEvent = {
          ...
          choices: helpEvent.choices.map(c => ({
            text: c.text,
            resultText: '',
            effects: {},
            customEffect: c.effect
          }))
        };
        this.phase = GamePhase.EVENT;
      }
          ...
          choices: [
            {
              ...
              effects: {},
              customEffect: (p) => {
                const result = opportunity.effect(p);
        ...
        choices: [
          {
            ...
            effects: {},
            customEffect: (p) => {
              const result = rm.haveChild(p);
              return result.success ? result.message : '这次没有成功怀孕。';
            }
          },
          {
            text: '再等等',
  ...
  buyProperty(propertyTypeId) {
    if (!this.player || !this.propertyManager) return { success: false, message: '游戏未开始' };
    const result = this.propertyManager.buyProperty(this.player, propertyTypeId);
    return result;
  }
  ...
  sellProperty(propertyId) {
    if (!this.player || !this.propertyManager) return { success: false, message: '游戏未开始' };
    const result = this.propertyManager.sellProperty(this.player, propertyId);
    return result;
  }

  // 获取可购买房产列表
  getAvailableProperties() {
    if (!this.player || !this.propertyManager) return [];
    return this.propertyManager.getAvailableProperties(this.player);
  }

  // 获取房产概览
  getPropertyOverview() {
    if (!this.player || !this.propertyManager) return null;
    return this.propertyManager.getPropertyOverview(this.player);
  }

  // 年度变化
  applyYearlyChanges() {
    // 工资年度增长（基础5% × 岔路带来的增长倍率，延迟期间不涨工资）
  ...
  calculateEnding() {
          ...
          try {
            if (endingDef.condition(p, stats)) {
              ending = {
                ...
                rarity: endingDef.rarity,
                icon: endingDef.icon
              };
              break;
            }
          } catch (e) {
            console.error('Ending condition error:', id, e);
          }
        }
        if (ending) break;
      }
    ...
    // 兜底：平凡安稳
    if (!ending) {
      ending = { id: 'peaceful', name: '🏠 平凡安稳型', description: '你过着平凡但安稳的一生。', rarity: 'common' };
    }
    ...
    const finalHooks = [];
    const maxHooks = 3;
    for (const h of endingHooks) {
      if (finalHooks.length >= maxHooks) break;
      finalHooks.push(h);
    }
    for (const h of choiceHooks) {
      if (finalHooks.length >= maxHooks) break;
      if (!finalHooks.includes(h)) {
        finalHooks.push(h);
      }
    }
    ending.parallelHooks = finalHooks;
    // 【P3】记录未解锁结局（用于结局页面展示"还有这些结局你没玩到"）
    if (typeof getLockedEndings !== 'undefined') {
      try {
        ending.lockedEndings = getLockedEndings(p);
      } catch (e) {
        ending.lockedEndings = [];
      }
    }
    // 【P1】记录结局到人生图鉴（只记录一次）
  ...
  calculateParallelHooks() {
    ...
    // 2. 基于玩家状态生成通用钩子（P4强化：覆盖更多状态，更精准）
    // 没有副业收入
    ...
    // 没买房（P4新增）
    const hasProperty = p.properties && p.properties.length > 0;
    if (!hasProperty && p.age >= 35 && p.savings > 50000) {
      hooks.push({ type: 'generic', text: PARALLEL_HOOKS.generic.noHouse });
    }
    // 没孩子（P4新增）
    const hasChildren = this.relationshipManager && this.relationshipManager.children && this.relationshipManager.children.length > 0;
    if (p.isMarried && !hasChildren && p.age >= 40) {
      hooks.push({ type: 'generic', text: PARALLEL_HOOKS.generic.noChildren });
    }
    // 没朋友（P4新增）
    const hasFriends = this.relationshipManager && this.relationshipManager.friends && this.relationshipManager.friends.length > 0;
    if (!hasFriends && p.age >= 35 && p.network < 30) {
      hooks.push({ type: 'generic', text: PARALLEL_HOOKS.generic.noFriends });
    }
    // 高储蓄但低投资（P4新增）
    if (p.savings > 200000 && p.investments < 50000 && p.age >= 35) {
      hooks.push({ type: 'generic', text: PARALLEL_HOOKS.generic.highSavingsLowInvest });
    }
    // 毕业后没学习（P4新增）
    const learnedCount = Object.keys(p.learnedSkills || {}).length;
    if (learnedCount === 0 && p.age >= 30) {
      hooks.push({ type: 'generic', text: PARALLEL_HOOKS.generic.noLearning });
    }
  ...
  load() {
    ...
    try {
      ...
      // 【修复】恢复人际关系和房产管理器数据
      if (data.relationshipManager && this.relationshipManager) {
        this.relationshipManager.deserialize(data.relationshipManager);
      }
      if (data.propertyManager && this.propertyManager) {
        this.propertyManager.deserialize(data.propertyManager);
      }
      return true;
    } catch (e) {
