...
const GamePhase = {
  ...
  RELATIONSHIPS: 'relationships', PROPERTY: 'property', GAME_OVER: 'game_over',
  YEAR_REVIEW: 'year_review', COLLECTION: 'collection', LAST_GAME_REVIEW: 'last_game_review'
};
class GameState {
  constructor() {
  ...
  buyProperty(propertyTypeId) {
    ...
    return this.propertyManager.buyProperty(this.player, propertyTypeId);
  }
  sellProperty(propertyId) {
    if (!this.player || !this.propertyManager) return { success: false, message: '游戏未开始' };
    return this.propertyManager.sellProperty(this.player, propertyId);
  }
  ...
  load() {
    ...
    try {
      ...
      if (data.relationshipManager && this.relationshipManager) this.relationshipManager.deserialize(data.relationshipManager);
      if (data.propertyManager && this.propertyManager) this.propertyManager.deserialize(data.propertyManager);
      return true;
