...
class Player {
  ...
  getMonthlyExpense() {
    ...
    const hasProperty = this.properties && this.properties.length > 0;
    const housingExpense = hasProperty ? this.monthlyMortgage : this.rent;
    const housingType = hasProperty ? 'mortgage' : 'rent';
    return {
      base: baseExp,
  ...
  recordMonthlyData(month, year) {
    ...
    this.monthlyHistory.push({
      month, year, age: this.age,
      income: income.total, incomeBreakdown: { ...income },
      expense: expense.total, expenseBreakdown: { ...expense },
      balance: income.total - expense.total,
      netWorth: this.getNetWorth(),
      savings: this.savings, investments: this.investments,
  ...
  applyEffects(effects) {
    ...
    if (effects.sideIncomeBase !== undefined) {
      this.sideIncome = effects.sideIncomeBase;
      applied.push(`副业收入¥${effects.sideIncomeBase}/月`);
    }
    if (effects.propertyValue !== undefined) {
      this.propertyValue += effects.propertyValue;
      this.hasHouse = true;
      applied.push(`获得房产¥${Math.round(effects.propertyValue).toLocaleString()}`);
    }
    if (effects.baseExpenseMultiplier) {
      this.modifiers.expenseMultiplier *= effects.baseExpenseMultiplier;
      applied.push(`基础支出×${effects.baseExpenseMultiplier}`);
    }
    if (effects.salaryStartMultiplier) {
      this.salaryStartMultiplier = effects.salaryStartMultiplier;
      applied.push(`职业起点工资×${effects.salaryStartMultiplier}`);
    }
    if (effects.salaryCapMultiplier) {
      this.salaryCapMultiplier = effects.salaryCapMultiplier;
      applied.push(`工资上限×${effects.salaryCapMultiplier}`);
    }
    return applied;
  }
  serialize() {
